...
__global__ void GetFractalInKernel(Uint32 *data,const int W,const int H,const int Group,const double Cx,const double Cy,const double EscapeRadius)
{
...
FractalParameter C_Real("C-Real Part",-0.615,-1,1),
				 C_Imag("C-Imag Part",-0.43,-1,1),
				 EscapeRadius("EscapeRaius",2,0.05,10);

void Fractal(const Posize &ps)
{
	Uint32 *Buffer=new Uint32[ps.Size()];

	Uint32 *DeviceData=nullptr;
	cudaMalloc((void**)&DeviceData,ps.Size()*sizeof(Uint32));
//	cudaMemset((void**)&DeviceData,100,ps.Size()*sizeof(Uint32));

	for (int i=0;i<(ps.h+1023)/1024;++i)
	{
		dim3 grid(ps.w);
		dim3 threads(min(ps.h-i*1024,1024));
		GetFractalInKernel << <grid, threads >> >(DeviceData,ps.w,ps.h,i,C_Real,C_Imag,EscapeRadius);
		cudaDeviceSynchronize();
	}
		
//	cudaError_t cudaStatus=cudaGetLastError();
//    if (cudaStatus!=cudaSuccess) 
//        DD[2]<<"addKernel launch failed: "<<cudaGetErrorString(cudaStatus)<<endl;
...
void FractalInit()
{
	...
	FractalParameter::SetWidgetWidth(700);
	C_Real.InitWidgets();
	C_Imag.InitWidgets();
